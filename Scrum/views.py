...
def burndown_sprint(request, sprint_id):
    buffer, sprint = construir_pdf_burndown(sprint_id)
...
def burndown_desarrollador(request, sprint_id):
    sprint = get_object_or_404(Sprint, pk=sprint_id)
    empleado = get_object_or_404(Empleado, Usuario=request.user)

    tareas = Tarea.objects.filter(HistoriaUsuario__Sprint=sprint, Empleado=empleado)
    if not tareas.exists():
        return FileResponse(io.BytesIO(b"No hay tareas asignadas."), content_type='text/plain')
    ...
    esfuerzo_por_dia = {fecha: 0 for fecha in fechas}
    avances = TareaAvance.objects.filter(tarea__in=tareas, horasDedicadas__gt=0).order_by('fechaAvance')
    ...
    with PdfPages(buffer) as pdf:
        ...
        if horas_estimadas != horas_reestimadas:
            ...
            for fecha in fechas:
                acumulado += esfuerzo_por_dia[fecha]
                reales_reestimadas.append(max(horas_reestimadas - acumulado, 0))
            plt2 = generar_burndown_chart(fechas, reales_reestimadas, horas_reestimadas,
                        f'{request.user.get_full_name()}  Horas Reestimadas - {sprint.nombresprint} ({inicio} al {fin}), día: {datetime.now().strftime("%Y-%m-%d")}')
            pdf.savefig()
            plt2.close()

    buffer.seek(0)
    Archivo =  f'{request.user.get_full_name()} - avance del {sprint.nombresprint} al día {datetime.now().strftime("%Y-%m-%d")}.pdf'
    return FileResponse(buffer, content_type='application/pdf', filename = Archivo)
